...
(function() {
  var DEFAULT_REPORT_FORMAT, Module, existsSync, fs, ibrik, istanbul, main, mkdirp, path, which;
  ...
  main = function(opts, callback) {
    var args, cmd, excludes, file, reportClassName, reportingDir, reports, runFn;
    cmd = opts._[0];
    file = opts._[1];
    args = opts._.slice(2);
    if (!file) {
    ...
    reports = [];
    reports.push(istanbul.Report.create(reportClassName, {
      dir: reportingDir
    }));
    runFn = function() {
    ...
    return istanbul.matcherFor({
      ...
      process.once('exit', function() {
        ...
        reports.forEach(function(report) {
          return report.writeReport(collector, true);
        });
        return callback();
      });
      return runFn();
